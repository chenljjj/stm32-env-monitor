...
### 6.1 三端长稳关联验证（2026-08-27）

最新 CSV 以真实设备 `sample=8` 至 `16075` 为范围，STM32、ESP32 和 MQTT JSON 各有 16068 条同序号记录，序号无缺口。ESP32 `uptime_ms` 从 `7084` 到 `16074084`，每相邻样本严格增加 1000 ms，覆盖 4 小时 27 分 47 秒。

全段 STM32 的 `rxerr=0`、`txerr=0`、`ackerr=0`、`retry=0`、`timeout=0`、`drop=0`、`qdrop=0/0`；ESP32 与 MQTT 记录中的传感器错误均为 0，说明协议收发、ACK 时序、去重和 MQTT 交接在该在线长稳片段内均未出现可观测异常。
...
STM32 收到合法 `NET_STATUS` 后，将 `flags` 缓存在链路状态中，并随下一份显示快照交给 `DisplayTask`。OLED 不直接访问网络，也不需要增加 I2C 设备；它只显示 UART 协议已经回传的状态：

```text
WIFI:ON MQTT:ON
```

其中 `WIFI:ON` 对应 bit0，`MQTT:ON` 对应 bit1。Wi-Fi 断开、已获取 IP 但 MQTT 尚未建连、MQTT 恢复的典型显示状态依次为 `--`、`W-`、`WM`，与 USART2 日志中的 `net` 字段一致。

该功能只修改 STM32 显示层：ESP32 在此前已发送相同格式的 `NET_STATUS`，因此无需为 OLED 排版变化重新构建或烧录 ESP32。OLED 布局已改为居中、统一字号的温度/湿度/照度三行与一行网络状态；重新烧录 STM32 后应在实板确认最终排版效果。
