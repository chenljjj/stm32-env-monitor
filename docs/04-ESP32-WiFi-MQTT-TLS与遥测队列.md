...
真实设备在 Wi-Fi 与 MQTT 均在线的条件下连续运行 4 小时 27 分 47 秒。关联 `sample=8` 至 `16075` 后，16068 条 MQTT JSON 均能与 STM32/ESP32 的同序号记录对应；所有 JSON 都可解析，三项有效标志均为 `true`，四类传感器错误计数均为 0。

末次网关统计为：

```text
net=WM wd=0 md=0 queue=0 submitted=16072 overflow=0
accepted=16072 puback=16072 puberr=0 abandoned=0 duplicate=0
```

该在线长稳片段内 MQTT 队列未积压、没有发布错误、受控丢弃或新的 Wi-Fi/MQTT 断线。此前断网恢复案例中的计数与本次在线基线应分别记录，不能混为一次测试结论。
...
6. 在已完成 4 小时 27 分 47 秒在线基线的基础上，扩展至至少 24 小时，记录重连次数、队列峰值、错误计数和堆内存趋势。
